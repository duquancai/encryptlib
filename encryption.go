...
func Encrypt(multiple cryptoType, plainText, key, iv []byte, mode mode, padding padding) ([]byte, error) {
	...
	case ISO10126:
		plainPadding = ISO10126Padding(plainText, bs)
	case ISO97971:
		plainPadding = ISO97971Padding(plainText, bs)
	case AnsiX923:
		plainPadding = AnsiX923Padding(plainText, bs)
	default:
...
func Decrypt(multiple cryptoType, cipherText, key, iv []byte, mode mode, padding padding) ([]byte, error) {
	...
	case ISO10126:
		dst = ISO10126UnPadding(dst, bs)
	case ISO97971:
		dst = ISO97971UnPadding(dst, bs)
	case AnsiX923:
		dst = AnsiX923UnPadding(dst, bs)
	}
